...
module.exports.generateCoupon = async function(req, res) {
  try {
    ...
    const { hash, pin } = await generateCoupon(jsonobj.email, jsonobj.amount);

    const hashString = hash.toString(); // Assuming `hash` is a BigNumber or a non-circular object
    
    res.send(`Coupon generated for email ${jsonobj.email} with discount ${jsonobj.amount}. Card Id: ${hashString}, Pin: ${pin}`);
  } catch (error) {
    console.error(error);
